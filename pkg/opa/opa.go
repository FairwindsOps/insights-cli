...
func CompareChecks(folder, org, token, hostName string, fileChecks []models.CustomCheckModel, deleteMissing bool) (CompareResults, error) {
	...
	results = compareChecks(fileChecks, apiChecks)
	return results, nil
}
...
func compareChecks(fileChecks []models.CustomCheckModel, apiChecks []opa.OPACustomCheck) CompareResults {
	var results CompareResults
...
// AddOPAChecksBranch builds the tree for OPA checks (V2 only)
func AddOPAChecksBranch(org, token, hostName string, tree treeprint.Tree) error {
	...
	for _, check := range checks {
		opaBranch.AddBranch(fmt.Sprintf("%s (v%.0f)", check.Name, check.Version))
	}
	return nil
}
